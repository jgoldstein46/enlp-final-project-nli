...
class NLI_Trainer:
    ...
    def run_training_loop(self):
        ...
        for i in range(self.params['epochs']):
            ...
            # Training
            if self.use_gru:
                self.nli_classifier.classifier.fit(self.train_batch_generator, epochs=1,
                                                   steps_per_epoch=self.train_data.shape[0] / self.params['batch_size'])
                loss, acc, recall, precision, f1 = self.nli_classifier.classifier.evaluate(self.dev_batch_generator,
